     ...
     <section className="pt-[3rem]  bg-black text-primary">
      <div className=" mx-auto px-4">
        ...
        <p className="text-center mb-8 max-w-3xl mx-auto">
          As a Frontend Developer, I rely on a variety of tools to streamline my workflow and enhance productivity.
          Here are some of the essential tools I use:
        </p>
        <ul className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-6">
          {toolsList.map((tool, index) => (
            <li key={index} className="flex flex-col items-center text-center p-4 bg-secondary/10 rounded-lg hover:shadow-lg transition backdrop-blur-md bg-white/10 rounded-full ">
              <img src={tool.img} alt={tool.title} className={clsx("w-16 h-16 mb-2", index === 4 && "bg-primary rounded-[100%] border")} />
